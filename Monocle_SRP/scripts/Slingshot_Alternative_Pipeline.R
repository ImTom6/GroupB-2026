# Alternative trajectory analysis using Slingshot
...
build_slingshot <- function(mat_log, n_clusters = 6, marker_genes = c()) {
  ...
  # extracting coordinates and pseudotime
  plot_df <- data.frame(
  ...
  plot_df$Pseudotime <- pt[, 1]
  
  if (all(is.na(plot_df$Pseudotime))) {
    stop("Slingshot failed: all pseudotime values are NA.")
  }
  
  max_pt <- max(plot_df$Pseudotime, na.rm = TRUE)
  plot_df$Pseudotime[is.na(plot_df$Pseudotime)] <- max_pt
  ...
  # this avoids needing the sce object later in the website
  ...
  # returning only plot-ready dataframe
  list(plot_df = plot_df)
}

# running Slingshot for Figure 2 and Figure 4
res_fetal <- build_slingshot(
  expr_mat_log_fetal,
  n_clusters = 6,
...
png("Figure2B_Slingshot.png", width = 2600, height = 700, res = 250)
grid.draw(fig2B)
dev.off()

png("Figure2AB_Slingshot_combined.png", width = 2600, height = 1800, res = 250)
grid.draw(fig2_combined)
dev.off()
...
png("Figure4B_Slingshot.png", width = 2600, height = 700, res = 250)
grid.draw(fig4B)
dev.off()

png("Figure4AB_Slingshot_combined.png", width = 2600, height = 1800, res = 250)
grid.draw(fig4_combined)
dev.off()
...
write.csv(
  plot_df_fetal,
  "Figure2_Slingshot_coordinates.csv",
  row.names = FALSE
)

write.csv(
  plot_df_org,
  "Figure4_Slingshot_coordinates.csv",
  row.names = FALSE
)

# saving .plot.rds plot objects only
saveRDS(fig2A, "Figure2A_Slingshot.plot.rds")
saveRDS(fig2B, "Figure2B_Slingshot.plot.rds")
saveRDS(fig2_combined, "Figure2AB_Slingshot.plot.rds")

saveRDS(fig4A, "Figure4A_Slingshot.plot.rds")
saveRDS(fig4B, "Figure4B_Slingshot.plot.rds")
saveRDS(fig4_combined, "Figure4AB_Slingshot.plot.rds")

slingshot_plot_objects <- list(
  Figure2A_plot = fig2A,
  Figure2B_plot = fig2B,
...
saveRDS(
  slingshot_plot_objects,
  "Slingshot_all_plot_objects.plot.rds"
)
...
cat("Saved PNG files:\n")
print(list.files(pattern = "Figure[24].*Slingshot.*png"))

cat("\nSaved CSV files:\n")
print(list.files(pattern = "Figure[24]_Slingshot.*csv"))

cat("\nSaved .plot.rds files:\n")
print(list.files(pattern = "Slingshot.*\\.plot\\.rds|Figure[24].*Slingshot.*\\.plot\\.rds"))

cat("\nThe Slingshot plots were saved as PNG and .plot.rds files.\n")
